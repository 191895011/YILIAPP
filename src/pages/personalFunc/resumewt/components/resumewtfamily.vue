  ...
  <div>
    ...
    <div class="resumeJob-container">
        <ul class="hrss-item-ul">
          ...
          <li @click="User_SelectYiLiJobTypesk">
            <span class="hrss-item-left">在伊利工作</span>
            <span class="hrss-item-right">{{FamilyForm.YiLiJobType}}</span>
          </li>
          <li>
            <span class="hrss-item-left">家庭成员员工ID</span>
            <span class="hrss-item-right">
              <input type="text" v-model.trim="FamilyForm.StaffID" placeholder="请输入" />
            </span>
          </li>
        </ul>
     </div>
     ...
     <!--与员工关系-->
       ...
       <mt-popup
        ...
        <mt-popup
         ...
         <!--在伊利工作-->
        <mt-popup
          v-model="YiLiJobTypeVisible"
          position="bottom"
          popup-transition="popup-fade">
            <mt-picker :slots="YiLiJobTypeSlots" @change="OnValuesYiLiJobTypeChange" value-key="name"  class="levelselect" showToolbar>
                 <div class="picker-toolbar-title">
                     <div class="usi-btn-cancel" @click="User_SelectYiLiJobTypesk">
                        取消
                     </div>
                     <div>
                     配偶是否有工作
                     </div>
                     <div class="usi-btn-sure" @click="User_SelectYiLiJobTypeSure">
                       确定
                     </div>
                  </div>
           </mt-picker>
...
export default {
  ...
  methods: {
    ...
    // 是否在伊利工作
     User_SelectYiLiJobTypesk () {
      this.YiLiJobTypeVisible = !this.YiLiJobTypeVisible
    },
    User_SelectYiLiJobTypeSure () {
      if (this.FamilyForm.YiLiJobTypeArr.length > 0) {
        this.FamilyForm.YiLiJobType = this.FamilyForm.YiLiJobTypeArr[0].name
      }
      this.YiLiJobTypeVisible = !this.YiLiJobTypeVisible
    },
    OnValuesYiLiJobTypeChange (picker, values) {
      this.FamilyForm.YiLiJobTypeArr = values
      console.log(this.FamilyForm.YiLiJobTypeArr)
    },
    KeepMsg () {
      this.$router.push({
        path: `/resumewt`
