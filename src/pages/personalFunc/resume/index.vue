<template>
  <div class="hrss-per resume-info">
    <div class="hrss-search resume-top">
      <h2>个人信息</h2>
      <img class="person-icon" :src="resume.photo?'data:image/png;base64,'+resume.photo:this.defIcon" />
      <div class="resume-top-msg">
        <span class="resume-name">{{resume.NAME}} [{{resume.emplid}}]
          <i class="resume-update el-icon-edit-outline" v-if="!emplid" @click="handleUpdate"></i>
        </span>
    ...
    getResume(emplId, role) {
      ...
      Resume(p).then(res => {
        if (res.status == "0") {
          ...
          sessionStorage.UserResume = JSON.stringify(res.JsonData)
