<template>
  <div class="hrss-per recruit-info">
    <div class="hrss-search">
      <h2 style="margin-bottom:0">招贤纳士详情</h2>
    </div>
      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item title="申请职位" name="3" v-if="recruitData.POSITION_NBR">
          <ul class="hrss-item-ul">
            <li>
              <span>国家/证件类型</span>
              <input class="hrss-item-input" type="text" v-model="selectShow" readonly @click="selectDatePopup=true" placeholder="请选择">
            ...
            <li>
              <span>证件号码</span>
              <input class="hrss-item-input" type="text" v-model="applyData.NATIONAL_ID" placeholder="请输入">
            </li>
    ...
    submitApply() {
      ...
      RecuitSave(this.applyData).then(res => {
        if (res.status == "0") {
          ...
          this.$messagebox.alert(res.message ? res.message : "系统异常");
        }
      });
    },
    getRecruitData(id) {
